...
`;function n(){let t=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&e.target.classList.add(`in-view`)})},{threshold:.12,rootMargin:`0px 0px -40px 0px`}),n=e.querySelector(`.portfolio`);if(!n)return;n.querySelectorAll(`.section`).forEach(e=>t.observe(e));let r=n.querySelector(`.footer`);r&&t.observe(r)}n(),r();function r(){let t=e.querySelector(`#mindmap-window`),n=e.querySelector(`#mindmap-lightbox`),r=e.querySelector(`#mindmap-lightbox-backdrop`),i=e.querySelector(`#mindmap-lightbox-close`);function a(){n&&(n.setAttribute(`aria-hidden`,`false`),n.classList.add(`is-open`),document.body.style.overflow=`hidden`,requestAnimationFrame(()=>i?.focus()))}function o(){n&&(n.classList.remove(`is-open`),n.setAttribute(`aria-hidden`,`true`),document.body.style.overflow=``)}t?.addEventListener(`click`,a),t?.addEventListener(`keydown`,e=>{let t=e;(t.key===`Enter`||t.key===` `)&&(t.preventDefault(),a())}),r?.addEventListener(`click`,o),i?.addEventListener(`click`,o),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&n?.classList.contains(`is-open`)&&o()})}